...
function taskTitle(t: Task) {
  switch (t.type) {
    ...
    case "winback_30d":
      return "Reativar 30 dias";
    default:
      return t.type;
  }
}

// Helper para formatar o dueAt vindo como Date ou Timestamp do Firestore
function formatDueDate(value: Date | Timestamp | null | undefined) {
  if (!value) return "-";
...
export default function TasksView() {
  ...
  const defaultMsg =
    "Passando só pra te dar retorno 😊 Quer algo mais doce ou mais amadeirado?";

  return (
    <div className="card">
      <h1 className="h1">Tarefas</h1>
      <div className="muted" style={{ marginBottom: 10 }}>
        (MVP) As tarefas automáticas entram na próxima fase com Cloud Functions.
        Aqui você já consegue executar as ações.
      </div>
          ...
          {items.map((t) => {
            ...
            return (
              <tr key={t.id}>
                ...
                <td className="muted">{formatDueDate(t.dueAt as any)}</td>
                <td>
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                    ...
                    <button className="btn" onClick={() => done(t.id)}>
                      Concluir
                    </button>
                  </div>
                </td>
      ...
      {items.length === 0 ? (
        <div className="muted">Sem tarefas abertas.</div>
      ) : null}
    </div>
  );
}
